...
const ResourceLoader = {
  ...
  async afterUserSignIn( user ) {
    ...
    getRegistry().commit('INIT_REGISTRY');
  ...
  afterUserSignOut() {
    ...
    /* Reset registry */
    getRegistry().commit('RESET_REGISTRY');
  ...
  attachTasklist( /* tasklist_id, is_multi_tasklist_attach */ ) {
    ...
    // let tasklist_list_state = getRegistry().$state.res.tasklist || {}
    // // TODO: maybe better: getRegistry().$state.sync[ '/tasklist/' + tasklistId + '/meta' ]
    //
